...
$(() => {
  ...
  let getIP = (url: string): string => {
    let ip = dns.lookup(
      url,
      (error: string, addresses: string, family: string) => {
        return addresses;
      }
    );
    return ip;
  ...
  let checkAddr = (param: string): boolean | string => {
    if (typeof param === "undefined" || !getIP(param)) return false;
    else return true;
  };
  ...
  let scanHost = () => {
    ...
    while (start <= end) {
      ((port) => {
        ...
        s.connect(p, ip, () => {
          $scanResult.addClass("result_port-open").append(`OPEN: ${p} <br />`);
        });

        s.on("data", (data: object) => {
          $scanResult.append(`DATA: ${data}`);
        });
